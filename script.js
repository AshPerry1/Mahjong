...
document.addEventListener('DOMContentLoaded', function() {
    ...
    prefersReducedMotion.addEventListener('change', function(e) {
        if (e.matches) {
            ...
            // Restart animations if user prefers motion
...
function initializeAnimations() {
    ...
    if (typeof rotateTestimonials === 'function') {
        window.testimonialInterval = setInterval(rotateTestimonials, 10000);
    }
    
    // Banner rotation
    if (typeof rotateBanner === 'function') {
        window.bannerInterval = setInterval(rotateBanner, 3000);
    }
}
